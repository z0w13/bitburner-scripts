...
export async function main(ns: NS): Promise<void> {
  ...
  if (flags["autobuy"]) {
    const args = [
      "--nextstep",
      flags["autobuy-nextstep"] ? "true" : "false",
      "--min-ram",
      flags["autobuy-minram"],
      "--interval",
      flags["autobuy-interval"],
    ]
  ...
  if (flags["autoinstall"]) {
    // TODO(zowie): also pass script flags
    const args = ["--interval", flags["autoinstall-interval"], "--host", flags["host"]]
...
async function runPlan(ns: NS, targetHost: string): Promise<void> {
  ...
  const hackWeakenCommand = {
    name: "hack-weaken",
    command: getWeakenCommand(ns, target, player, hackCommand.command.security),
  }
  const growCommand = { name: "grow", command: getGrowCommand(ns, target, player) }
  const growWeakenCommand = {
    name: "grow-weaken",
    command: getWeakenCommand(ns, target, player, growCommand.command.security),
  }
  ...
  while (true) {
    ...
    for (let i = 0; i < runAmount; i++) {
      ...
      const hackWeakenPromise = delayedCommand(
        ns,
        delays[hackWeakenCommand.name] + INTERVAL * 1,
        hackWeakenCommand,
        batchId,
      )
      const growPromise = delayedCommand(ns, delays[growCommand.name] + INTERVAL * 2, growCommand, batchId)
      const growWeakenPromise = delayedCommand(
        ns,
        delays[growWeakenCommand.name] + INTERVAL * 3,
        growWeakenCommand,
        batchId,
      )
      ...
      ns.printf(
        JSON.stringify({
          ...
          pct: (batchSuccess / batchTotal) * 100 ?? 0,
          id: batchId,
        }),
      )
...
async function delayedCommand(
  ns: NS,
  delay: number,
  { name, command }: { name: string; command: Command },
  id = 0,
): Promise<void> {
  ...
  switch (command.script) {
    case SCRIPT_WEAKEN: {
      ...
      if (weakenTime > command.time + 10) {
        throw Error(
          ns.sprintf(
            "%s, %s, getWeakenTime mismatch: %.2f more than 10ms off %.2f",
            command.target,
            name,
            command.time,
            weakenTime,
    ...
    case SCRIPT_GROW: {
      ...
      if (growTime > command.time + 10) {
        throw Error(
          ns.sprintf(
            "%s, %s, getGrowTime mismatch: %.2f more than 10ms off %.2f",
            command.target,
            name,
            command.time,
            growTime,
    ...
    case SCRIPT_HACK: {
      ...
      if (command.time - 10 > hackTime) {
        throw Error(
          ns.sprintf(
            "%s, %s, getHackTime mismatch: %.2f more than 10ms off %.2f",
            command.target,
            name,
            command.time,
            hackTime,
      ...
      if (moneyAvailable < moneyMax * 0.95) {
        throw Error(
          ns.sprintf(
            "%s, %s, Money mismatch: %s less than 95%% of %s",
            command.target,
            name,
            ns.nFormat(moneyAvailable, "$0,0a"),
            ns.nFormat(moneyMax, "$0,0a"),
          ),
        )
      }
      break
